...
{
	fprintf(stderr, "Usage: ... wred limit BYTES [min BYTES] [max BYTES] avpkt BYTES [burst PACKETS]\n");
...
static int red_parse_opt(struct qdisc_util *qu, int argc, char **argv, struct nlmsghdr *n)
{
	...
	if (!opt.limit || !avpkt) {
		fprintf(stderr, "WRED: Required parameter (limit, avpkt) is missing\n");
		return -1;
	}
	...
	if ((wlog = tc_red_eval_ewma(opt.qth_min, burst, avpkt)) < 0) {
		fprintf(stderr, "WRED: failed to calculate EWMA constant.\n");
		return -1;
	}
	if (wlog >= 10)
		fprintf(stderr, "WRED: WARNING. Burst %d seems to be too large.\n", burst);
	opt.Wlog = wlog;
	if ((wlog = tc_red_eval_P(opt.qth_min, opt.qth_max, probability)) < 0) {
		fprintf(stderr, "WRED: failed to calculate probability.\n");
		return -1;
	}
	opt.Plog = wlog;
	
	if ((wlog = tc_red_eval_idle_damping(opt.Wlog, avpkt, rate, sbuf)) < 0) {
		fprintf(stderr, "WRED: failed to calculate idle damping table.\n");
		return -1;
	}
	opt.Scell_log = wlog;
